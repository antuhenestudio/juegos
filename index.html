...
<head>
  ...
  <title>Mente en Juego — Estimulación cognitiva para cada etapa</title>
  <meta name="description" content="PequeMundo (niños 3-11: pensar, hablar, mover y ahorrar) y MenteActiva (adultos: memoria, atención, cálculo y lenguaje con accesibilidad)." />
...
// ============================================================
// PequeMundo v3 — Plataforma de estimulación cognitiva
//  · Módulo NIÑOS (3-5 / 6-8 / 9-11): Pensar, Hablar, Mover
//    + nueva área AHORRAR (educación financiera)
//    + Panel de padres con lectura científica honesta
//  · Módulo ADULTOS "MenteActiva" (bienestar 55+ / apoyo en
//    rehabilitación): memoria, atención, cálculo y lenguaje,
//    con accesibilidad (alto contraste, letra grande, audio,
//    sin temporizadores, dificultad adaptativa) y panel de
//    cuidador.
// ============================================================
...
// ---------- audio-guía (Web Speech API, con protección) ----------
...
function Figura({ id, edad = "3-5", className = "" }) {
  const d = edad !== "3-5"; // versión detallada
  const g = d ? 2.5 : 4.5;
...
const AREAS = {
  ...
  economia: { nombre: "Ahorrar", icono: "💰", color: "bg-pink-500", suave: "bg-pink-100", texto: "text-pink-700", desc: "Dinero, ahorro y decisiones",
    habilidad: "Introduce nociones de ahorro, valor del dinero y diferencia entre necesidades y deseos. La OCDE recomienda comenzar la educación financiera en edades tempranas, y estudios longitudinales como el de Dunedin (Moffitt y colegas) asociaron el autocontrol en la infancia con mejores resultados financieros y de salud en la adultez." },
};

const JUEGOS = [
  { id: "memoria", nombre: "Memoria de pares", icono: "🃏", area: "cognitiva", edades: ["3-5", "6-8", "9-11"] },
  { id: "patrones", nombre: "¿Qué sigue?", icono: "🔮", area: "cognitiva", edades: ["3-5", "6-8", "9-11"] },
  { id: "matematicas", nombre: "Súper cuentas", icono: "➕", area: "cognitiva", edades: ["6-8", "9-11"] },
  { id: "palabras", nombre: "Nombra la figura", icono: "🗣️", area: "lenguaje", edades: ["3-5", "6-8"] },
  { id: "letras", nombre: "Primera letra", icono: "🔤", area: "lenguaje", edades: ["6-8", "9-11"] },
  { id: "atrapa", nombre: "Atrapa al bichito", icono: "🐞", area: "psicomotor", edades: ["3-5", "6-8", "9-11"] },
  { id: "alcancia", nombre: "La alcancía", icono: "🐷", area: "economia", edades: ["6-8", "9-11"] },
  { id: "necesito", nombre: "¿Necesito o quiero?", icono: "🤔", area: "economia", edades: ["6-8", "9-11"] },
];
...
function JuegoMemoria({ edad, alTerminar }) {
  const pares = edad === "3-5" ? 3 : edad === "6-8" ? 6 : 8;
...
function JuegoPatrones({ edad, alTerminar }) {
  ...
  const generarRonda = useCallback(() => {
    ...
    let base;
    const tipo = azar(3);
    if (tipo === 0) base = [a, b, a, b, a, b, a];
    else if (tipo === 1) base = [a, a, b, b, a, a, b];
    else base = [a, b, c, a, b, c, a];
    const largo = edad === "9-11" ? 6 : 5;
    const sec = base.slice(0, largo);
    const resp = base[largo];
...
function JuegoMatematicas({ edad, alTerminar }) {
  const TOTAL = 10;
  const generar = () => {
    if (edad === "9-11" && azar(2) === 0) {
      const n1 = azar(8) + 2, n2 = azar(8) + 2;
      const resp = n1 * n2;
      const ops = new Set([resp]);
      while (ops.size < 3) {
        const dd = resp + (azar(13) - 6);
        if (dd > 0 && dd !== resp) ops.add(dd);
      }
      return { n1, n2, simbolo: "×", resp, opciones: mezclar([...ops]) };
    }
    const tope = edad === "9-11" ? 40 : 20;
    let n1 = azar(tope) + 1, n2 = azar(tope) + 1;
    const suma = azar(2) === 0;
    ...
    return { n1, n2, simbolo: suma ? "+" : "−", resp, opciones: mezclar([...ops]) };
  };
  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:gap-6">
      ...
      <div className="rounded-3xl bg-white px-8 py-6 shadow-md sm:px-10 sm:py-8">
        <p className="text-5xl font-black text-slate-800 sm:text-6xl">{p.n1} {p.simbolo} {p.n2}</p>
      </div>
      <div className="flex gap-3 sm:gap-4">
        {p.opciones.map((op) => (
  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:gap-6">
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => hablar("¿Cómo se llama esto?")} aria-label="Escuchar consigna"
          className="rounded-full bg-amber-200 px-3 py-1 text-lg shadow active:scale-90">🔊</button>
      </div>
...
function JuegoAtrapa({ edad, alTerminar }) {
  const DURACION = 30;
  const velocidad = edad === "3-5" ? 2000 : edad === "6-8" ? 1200 : 900;
  const tamano = edad === "3-5" ? "h-20 w-20 sm:h-24 sm:w-24" : edad === "6-8" ? "h-12 w-12 sm:h-16 sm:w-16" : "h-10 w-10 sm:h-12 sm:w-12";
  const meta = edad === "3-5" ? 10 : edad === "6-8" ? 15 : 18;
...
// -------- NUEVO: La alcancía (educación financiera) --------
function JuegoAlcancia({ edad, alTerminar }) {
  const TOTAL = 6;
  const monedas = edad === "9-11" ? [1, 2, 5, 10, 20] : [1, 2, 5];
  const generarMeta = () => (edad === "9-11" ? 15 + azar(46) : 5 + azar(8));
  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:gap-6">
      <p className="text-center text-lg font-bold text-slate-600 sm:text-xl">
        Llená la alcancía con <span className="text-pink-600">${meta}</span> justos 🐷
      </p>
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {monedas.map((v) => (
          <button key={v} onClick={() => agregar(v)}
            className="relative h-16 w-16 transition-transform active:scale-90 sm:h-20 sm:w-20" aria-label={`Moneda de ${v}`}>
            <Figura id="moneda" edad={edad} className="h-full w-full" />
            <span className="absolute inset-0 flex items-center justify-center pt-1 text-lg font-black text-amber-900 sm:text-xl">{v}</span>
          </button>
        ))}
        <button onClick={() => { setSuma(0); setFallo(true); }}
          className="rounded-2xl bg-slate-200 px-4 text-sm font-black text-slate-600 active:scale-95">Vaciar</button>
      </div>
...
// -------- NUEVO: ¿Necesito o quiero? (educación financiera) --------
const ITEMS_NQ = [
...
function JuegoNecesito({ alTerminar }) {
  const TOTAL = 8;
  const [orden] = useState(() => mezclar(ITEMS_NQ).slice(0, TOTAL));
  const [num, setNum] = useState(0);
  const [puntos, setPuntos] = useState(0);
  const [marca, setMarca] = useState(null);

  const item = orden[num];

  const responder = (eleccion) => {
    if (marca) return;
    const ok = eleccion === item.tipo;
    setMarca({ ok, por: item.por });
    const nuevos = ok ? puntos + 1 : puntos;
    if (ok) setPuntos(nuevos);
    setTimeout(() => {
      if (num + 1 >= TOTAL) alTerminar(nuevos, TOTAL);
      else { setNum(num + 1); setMarca(null); }
    }, 1600);
  };

  return (
    <div className="flex flex-col items-center gap-5 sm:gap-6">
      <p className="text-center text-lg font-bold text-slate-600 sm:text-xl">¿Es algo que necesitás o algo que querés? 🤔</p>
      <div className="flex flex-col items-center gap-1 rounded-3xl bg-white px-10 py-6 shadow-md">
        <span className="text-7xl sm:text-8xl">{item.e}</span>
        <span className="text-xl font-black text-slate-700 sm:text-2xl">{item.p}</span>
      </div>
      <div className="flex gap-4">
        <button onClick={() => responder("necesito")}
          className={`rounded-full px-8 py-4 text-xl font-black shadow-md transition-transform active:scale-95 sm:text-2xl ${
            marca && item.tipo === "necesito" ? "bg-green-300 text-green-900" : "bg-sky-500 text-white"
          }`}>
          Lo necesito
        </button>
        <button onClick={() => responder("quiero")}
          className={`rounded-full px-8 py-4 text-xl font-black shadow-md transition-transform active:scale-95 sm:text-2xl ${
            marca && item.tipo === "quiero" ? "bg-green-300 text-green-900" : "bg-pink-500 text-white"
          }`}>
          Lo quiero
        </button>
      </div>
      {marca && (
        <p className={`max-w-sm text-center text-base font-bold sm:text-lg ${marca.ok ? "text-green-700" : "text-amber-700"}`}>
          {marca.ok ? "✅ " : "💡 "}{marca.por}
        </p>
      )}
      <p className="text-base font-bold text-pink-600 sm:text-lg">Elección {num + 1} de {TOTAL} · Puntos: {puntos}</p>
    </div>
  );
}

// -------- Panel para padres (con lectura científica honesta) --------
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-sky-100 p-4 sm:gap-8 sm:p-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-base font-bold text-slate-500 sm:text-lg">Jugar, pensar, hablar, mover y ahorrar</p>
        </div>
        <div className="flex w-full max-w-sm flex-col gap-4 rounded-3xl bg-white p-5 shadow-lg sm:p-6">
          ...
          <div className="flex gap-2">
            {["3-5", "6-8", "9-11"].map((e) => (
              <button key={e} onClick={() => setEdadInput(e)}
                className={`flex-1 rounded-2xl py-3 text-base font-black transition-transform active:scale-95 sm:text-lg ${
          ...
          <button onClick={crearPerfil} disabled={!nombreInput.trim() || !edadInput}
          ...
          <div className="rounded-3xl bg-sky-50 p-3 sm:p-4">
            {resultado ? (
              ...
              <div key={claveJuego}>
                ...
                {juegoActivo.id === "matematicas" && <JuegoMatematicas edad={perfil.edad} alTerminar={terminarJuego} />}
                ...
                {juegoActivo.id === "alcancia" && <JuegoAlcancia edad={perfil.edad} alTerminar={terminarJuego} />}
                {juegoActivo.id === "necesito" && <JuegoNecesito alTerminar={terminarJuego} />}
              </div>
            )}
          </div>
        </div>
...
// Estimulación cognitiva accesible: sin temporizadores,
// botones grandes, audio, alto contraste, dificultad adaptativa
// ============================================================
...
// Tema visual según configuración de accesibilidad
function tema(cfg) {
  if (cfg.contraste) {
    return {
...
// -------- Ejercicio adulto: Memoria de pares (adaptativo) --------
...
// -------- Ejercicio adulto: Atención (buscar objetivos, sin tiempo) --------
function EjAtencionAdulto({ nivel, cfg, alTerminar }) {
  ...
  const [estado, setEstado] = useState(celdas);
...
// -------- Ejercicio adulto: Cálculo adaptativo --------
...
// -------- Ejercicio adulto: Lenguaje (nombrar objetos, con audio) --------
function EjLenguajeAdulto({ nivel, cfg, alTerminar }) {
  const t = tema(cfg);
  const TOTAL = 8;
  const banco = VOCABULARIO;
  const numOpciones = nivel >= 4 ? 4 : 3;
  const generar = useCallback(() => {
    const item = banco[azar(banco.length)];
    const ops = new Set([item.p]);
    while (ops.size < numOpciones) ops.add(banco[azar(banco.length)].p);
    return { ...item, opciones: mezclar([...ops]) };
  }, [numOpciones]); // eslint-disable-line
    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-8 bg-gradient-to-b from-sky-100 to-stone-100 p-6">
      ...
      <div className="flex w-full max-w-lg flex-col gap-5 sm:flex-row">
        <button onClick={() => setModo("nino")}
          ...
          <span className="text-center font-bold text-slate-500">Niñas y niños de 3 a 11 años: pensar, hablar, mover y ahorrar</span>
        </button>
