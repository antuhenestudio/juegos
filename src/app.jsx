...
function aplicarRegion(pais) {
  const cambiar = (lista, de, a, sil) => {
    const it = lista.find((x) => x.p === de || x.p === a);
    if (it) { it.p = a; it.s = sil; }
  };
  TXT.plata = ["ar", "uy"].includes(pais) ? "plata" : "dinero";
  if (["ar", "uy", "cl"].includes(pais)) cambiar(PALABRAS_DIFICIL, "fresa", "frutilla", ["fru", "ti", "lla"]);
  else cambiar(PALABRAS_DIFICIL, "frutilla", "fresa", ["fre", "sa"]);
  if (["mx", "es", "pe"].includes(pais)) cambiar(PALABRAS_MEDIO, "banana", "plátano", ["plá", "ta", "no"]);
  else cambiar(PALABRAS_MEDIO, "plátano", "banana", ["ba", "na", "na"]);
}
